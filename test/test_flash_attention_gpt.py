...
# 首先添加Flash Attention的实现
class FlashAttention(nn.Module):
    """Flash Attention实现 - 优化版本的注意力计算"""
    ...
    def _flash_attention_forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
        ...
        这里简化实现，实际的Flash Attention需要更复杂的优化
        """
        batch_size, seq_len, n_head, head_dim = q.shape
        
        # 传统注意力计算（简化版）
        att = torch.matmul(q, k.transpose(-2, -1)) * (1.0 / math.sqrt(head_dim))
    ...
    def forward(self, x: torch.Tensor, use_cache: bool = False, 
        ...
        y = self._flash_attention_forward(q, k, v)
        ...
        return y, None  # Flash Attention不使用缓存
...
class GPTAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, use_cache: bool = False, 
        ...
        # 使用Flash Attention优化计算
        if self.use_flash_attention and seq_len <= 512:  # Flash Attention通常适用于较小序列
            # 简化版本的Flash Attention计算
...
#     print("=" * 60)
#     print("GPT 聊天模型实现 (包含Flash Attention)")
#     print("=" * 60)
